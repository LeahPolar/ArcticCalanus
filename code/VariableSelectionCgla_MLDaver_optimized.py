...
def load_and_prepare_data(file_path):
    ...
    # 数据验证
    required_columns = ['Cgla_c1c2', 'year', 'SIC', 'CHL', 'SST', 'Sal', 'nitrat', 'mlotst_glor', 
...
def calbestComb(num, df, feature_names, cycle_years):
    ...
    for combination in tqdm(itertools.combinations(feature_names, num), 
        ...
        for cycle_year in cycle_years:
            ...
            y_train = df.loc[train_mask, 'Cgla_occur']
            X_test = df.loc[test_mask, remaining_features]
            y_test = df.loc[test_mask, 'Cgla_occur']
...
def main():
    ...
    for i in range(9, 5, -1):
        ...
        try:
            ...
            auc, tss = plotGCV(list(outcome[1][0]), "Cgla_MLDaver", df, cycle_years)
