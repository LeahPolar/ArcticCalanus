...
for subdir in os.listdir(input_path):
    if re.fullmatch(r"\d{4}", subdir):
        ...
        summer_tif_files.extend(sorted(glob.glob(os.path.join(year_folder, "CglaSuitability*.tif"))))

group1 = []  # 1993-2007
group2 = []  # 2008-2022

for tif in summer_tif_files:
    basename = os.path.basename(tif)
    date_str = basename.replace("CglaSuitability", "").replace(".tif", "")
    year = int(date_str[:4])
    month = int(date_str[4:6])
    if month not in [7, 8, 9]:
        continue  # 只保留7-9月
    if 1993 <= year <= 2007:
        group1.append(tif)
    elif 2008 <= year <= 2022:
...
for arr, fname in zip(
    ...
    ["CglaSuitability1993-2007mean_summer.tif", "CglaSuitability2008-2022mean_summer.tif", "CglaSuitability2008-2022minus1993-2007_summer.tif", "CglaSuitability1993-2022mean_summer.tif"]):
...
# 读取summer平均和差值影像用于绘图
with rasterio.open(os.path.join(output_path, "CglaSuitability1993-2022mean_summer.tif")) as src:
    mean_all_img = src.read(1)
diff_img = None
with rasterio.open(os.path.join(output_path, "CglaSuitability2008-2022minus1993-2007_summer.tif")) as src:
    diff_img = src.read(1)
...
axes[0].set_title('Mean Suitability 1993-2022 (July-September)', pad=30)
...
axes[1].set_title('Suitability Change (2008-2022 minus 1993-2007, July-September)', pad=30)
