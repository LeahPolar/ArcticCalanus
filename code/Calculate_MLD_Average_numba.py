...
def process_one_day(glorys_ds, day_idx, mld_file, depth_arr, varname, target_date):
    # 检查输出文件是否已存在
    output_dir = os.path.join(OUTPUT_BASE, varname)
    output_path = os.path.join(output_dir, f"{varname}MLDaver{target_date:%Y%m%d}.tif")
    if os.path.exists(output_path):
        return None  # 文件已存在，跳过计算
    
    data = glorys_ds.variables[varname][day_idx, :, :, :]
...
def main():
    for year in tqdm(range(1994, 2023), desc='年份进度'):
        ...
        for i in tqdm(range(n_days), desc=f'{year}年天数进度', leave=False):
            ...
            for var, ncvar in VARIABLES.items():
                out_arr = process_one_day(ds, i, mld_file, depth_arr, ncvar, target_date)
                if out_arr is not None:  # 只有当文件不存在时才计算和输出
