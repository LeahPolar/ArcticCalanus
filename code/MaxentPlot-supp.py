...
X = df[['CHL', 'SST', 'SIC', 'nitrat', 'mlotst_glor', 'Sal']]
y = df["Cgla_occur"]
...
def MaxentPredict(outputyear, maxent):
    ...
    output_path = f"/home/yytan/Maxent/Cgla-supp/{outputyear}"
    ...
    for i in tqdm(range(days_in_year)):
        ...
        # u = ds.variables['u'][i,0,:,:]
        # v = ds.variables['v'][i,0,:,:]
        # npp = ds.variables['npp'][i,0,:,:]
        ...
        # filepath2 = f"/data/oceandata/GLOBAL_MULTIYEAR_BGC_001_029/{target_date.year:04d}/{target_date.month:02d}/"
        # filename2 = f"mercatorfreebiorys2v4_global_mean_{target_date.year:04d}{target_date.month:02d}{target_date.day:02d}.nc"
        # if not os.path.exists(filepath2+filename2):
        #     continue
        # bgc = nc.Dataset(filepath2+filename2)
        # o2 = bgc.variables["o2"][0, 0, -201:,:]
        # po4 = bgc.variables["po4"][0, 0, -201:,:]
        # si = bgc.variables["si"][0, 0, -201:,:]
        Envivar = pd.DataFrame(columns=X.columns)
        Envivar["SIC"] = SIC.reshape(-1)
        Envivar['CHL'] = CHL.reshape(-1)
        Envivar['SST'] = SST.reshape(-1)
        # Envivar['SIT'] = SIT.reshape(-1)
        Envivar['Sal'] = Sal.reshape(-1)
        # Envivar['u'] = u.reshape(-1)
        # Envivar['v'] = v.reshape(-1)
        # Envivar['npp'] = npp.reshape(-1)
        Envivar['nitrat'] = nitrat.reshape(-1)
        Envivar['mlotst_glor'] = mld.reshape(-1)
        # Envivar['o2_surface'] = o2.reshape(-1)
        # Envivar['po4_surface'] = po4.reshape(-1)
        # Envivar['si_surface'] = si.reshape(-1)
...
def SeasonalOutput(year, outputpath):
    file_pattern = f"/home/yytan/Maxent/Cgla-supp/{year}/CglaSuitability*.tif"
    ...
    plt.savefig(f"/home/yytan/Figures/Cgla-supp/Cgla{year}Siutability.jpg", dpi=300)
    plt.close("all")


outputpath = "/home/yytan/Maxent/Cgla-supp/seasonal"
...
for season in seasons:
    ...
    input_dir = "/home/yytan/Maxent/Cgla-supp/seasonal"
...
for j, variable in enumerate(title):
    ...
    plt.savefig(f"/home/yytan/Figures/Cgla-supp/CglaSeasonal{variable}Atlantic_nouv.jpg", dpi=300)
...
for j, variable in enumerate(title):
    ...
    cbar = fig.colorbar(img, ax=axe, orientation='horizontal', fraction=0.046, pad=0.1, extend=ext)
    cbar.set_label(f'Cgla 1993-2022 Suitability {variable}', fontsize=12)
    plt.savefig(f"/home/yytan/Figures/Cgla-supp/CglaSeasonal{variable}Pacific_nouv.jpg", dpi=300)
